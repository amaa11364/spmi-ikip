...
<style>
    ...
    .evaluasi-icon.ami { background-color: #e3f2fd; color: #1976d2; }
    .evaluasi-icon.edom { background-color: #e8f5e9; color: #388e3c; }
    .evaluasi-icon.evaluasi_layanan { background-color: #fff3e0; color: #f57c00; }
    .evaluasi-icon.evaluasi_kinerja { background-color: #f3e5f5; color: #7b1fa2; }
    ...
    .badge-aktif, .badge-selesai { background-color: #d4edda; color: #155724; }
...
<div class="container-fluid px-3 px-md-4">
    ...
    <div class="filter-section">
        ...
        <div class="filter-tabs">
            <a href="{{ route('spmi.evaluasi-full.index') }}" class="filter-tab {{ !request('tipe') || request('tipe') == 'all' ? 'active' : '' }}">
                Semua Evaluasi
            </a>
            <a href="{{ route('spmi.evaluasi-full.index', ['tipe' => 'ami']) }}" class="filter-tab {{ request('tipe') == 'ami' ? 'active' : '' }}">
                AMI
            </a>
            <a href="{{ route('spmi.evaluasi-full.index', ['tipe' => 'edom']) }}" class="filter-tab {{ request('tipe') == 'edom' ? 'active' : '' }}">
                EDOM
            </a>
            <a href="{{ route('spmi.evaluasi-full.index', ['tipe' => 'evaluasi_layanan']) }}" class="filter-tab {{ request('tipe') == 'evaluasi_layanan' ? 'active' : '' }}">
                Evaluasi Layanan
            </a>
            <a href="{{ route('spmi.evaluasi-full.index', ['tipe' => 'evaluasi_kinerja']) }}" class="filter-tab {{ request('tipe') == 'evaluasi_kinerja' ? 'active' : '' }}">
                Evaluasi Kinerja
            </a>
        </div>
        
        <!-- Search and Filter Form -->
        <form method="GET" action="{{ route('spmi.evaluasi-full.index') }}">
            <div class="row g-3">
                <div class="col-md-3">
                                ...
                                <div class="action-buttons position-relative">
                                    ...
                                    <div class="position-relative">
                                        ...
                                        <div class="upload-inline-modal" id="uploadModalEvaluasi{{ $item->id }}">
                                            <form action="{{ route('spmi.evaluasi-full.upload', $item->id) }}" method="POST" enctype="multipart/form-data" class="upload-inline-form" id="uploadFormEvaluasi{{ $item->id }}">
                                                @csrf
                                                <div class="mb-2">
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('spmi.evaluasi-full.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                        @csrf
                                        @method('DELETE')
...
<script>
    ...
    function viewEvaluasi(id) {
        ...
        const url = '{{ route("spmi.evaluasi-full.ajax.detail", ":id") }}'.replace(':id', id);
    ...
    function editEvaluasi(id) {
        ...
        const url = '{{ route("spmi.evaluasi-full.ajax.edit-form", ":id") }}'.replace(':id', id);
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    jQuery('#editForm').attr('action', '{{ route("spmi.evaluasi-full.update", ":id") }}'.replace(':id', id));
