...
@section('title', 'Detail Penetapan SPMI')
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="{{ route('spmi.penetapan.index') }}">
                    <i class="fas fa-folder-open me-1"></i> Repository Penetapan
                </a>
            </li>
            <li class="breadcrumb-item active" aria-current="page">
                Detail: {{ $penetapan->nama_komponen }}
            </li>
        </ol>
    </nav>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="detail-card">
                ...
                <div class="detail-header">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <h4 class="mb-2">{{ $penetapan->nama_komponen }}</h4>
                            <div class="d-flex flex-wrap gap-2">
                                <span class="badge bg-info">{{ $penetapan->kode_penetapan }}</span>
                                <span class="badge bg-secondary">{{ $penetapan->tahun }}</span>
                                <span class="badge bg-{{ $penetapan->status_color }}">{{ $penetapan->status_label }}</span>
                                <span class="badge bg-{{ $penetapan->status_dokumen_color }}">{{ $penetapan->status_dokumen_label }}</span>
                            </div>
                        </div>
                        <div class="btn-group">
                            <a href="{{ route('spmi.penetapan.edit', $penetapan->id) }}" class="btn btn-outline-primary">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="info-group">
                            <div class="info-label">
                                <i class="fas fa-cogs me-2 text-primary"></i> Tipe Penetapan
                            </div>
                            <div class="info-value">
                                {{ $penetapan->tipe_penetapan_label }}
                            </div>
                        </div>
                        ...
                        <div class="info-group">
                            ...
                            <div class="info-value">
                                {{ $penetapan->penanggung_jawab ?? 'Belum ditentukan' }}
                            </div>
                        </div>
                        ...
                        <div class="info-group">
                            ...
                            <div class="info-value">
                                {{ $penetapan->unitKerja->nama ?? 'Tidak ada' }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="info-group">
                            ...
                            <div class="info-value">
                                {{ $penetapan->iku->nama ?? 'Tidak ada' }}
                            </div>
                        </div>
                        
                        <div class="info-group">
                            <div class="info-label">
                                <i class="fas fa-calendar me-2 text-primary"></i> Tanggal Penetapan
                            </div>
                            <div class="info-value">
                                {{ $penetapan->tanggal_penetapan ? $penetapan->tanggal_penetapan->format('d/m/Y H:i') : 'Belum ditetapkan' }}
                            </div>
                        </div>
                        
                        <div class="info-group">
                            <div class="info-label">
                                <i class="fas fa-search me-2 text-primary"></i> Terakhir Review
                            </div>
                            <div class="info-value">
                                {{ $penetapan->tanggal_review ? $penetapan->tanggal_review->format('d/m/Y H:i') : 'Belum direview' }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Deskripsi -->
                @if($penetapan->deskripsi)
                <div class="info-group">
                    <div class="info-label">
                        <i class="fas fa-align-left me-2 text-primary"></i> Deskripsi
                    </div>
                    <div class="info-value">
                        <p class="mb-0">{{ $penetapan->deskripsi }}</p>
                    </div>
                </div>
                @endif

                <!-- Catatan Verifikasi -->
                @if($penetapan->catatan_verifikasi)
                <div class="info-group">
                    <div class="info-label">
            ...
            <div class="detail-card">
                ...
                <!-- Quick Upload Form -->
                <form action="{{ route('spmi.penetapan.upload', $penetapan->id) }}" method="POST" enctype="multipart/form-data" class="upload-inline-form" id="quickUploadForm">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Upload File ke <strong>{{ $penetapan->nama_komponen }}</strong></label>
                    ...
                    <div class="row g-2">
                        <div class="col-md-8">
                            <input type="text" class="form-control" name="keterangan" placeholder="Keterangan (opsional)" value="Dokumen {{ $penetapan->nama_komponen }}">
                        </div>
                        <div class="col-md-4">
                    ...
                    <input type="hidden" name="jenis_dokumen" value="Penetapan SPMI">
                ...
                @else
                    <div class="text-center py-4">
                        ...
                        <p class="text-muted mb-3">Upload dokumen pertama untuk komponen ini</p>
                        <div class="d-flex justify-content-center gap-2">
                            <button class="btn btn-primary" onclick="toggleUploadForm()">
                                <i class="fas fa-paperclip me-1"></i> Upload Cepat
                            </button>
                            <a href="{{ route('upload.spmi-penetapan', $penetapan->id) }}" class="btn btn-outline-primary">
                                <i class="fas fa-upload me-1"></i> Upload Lengkap
                            </a>
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <ul class="list-unstyled mb-0">
                        ...
                        @if($penetapan->diperiksa_oleh)
                        <li class="mb-2">
                            <small class="text-muted">
                                <i class="fas fa-user-check me-2"></i>
                                <strong>Diperiksa oleh:</strong> {{ $penetapan->diperiksa_oleh }}
                            </small>
                        </li>
                        @endif
                        <li class="mb-2">
                            <small class="text-muted">
                                ...
                                <code class="small">{{ $penetapan->folder_path }}</code>
                            </small>
                        </li>
                        <li class="mb-2">
                            <small class="text-muted">
            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        ...
                        <a href="{{ route('spmi.penetapan.edit', $penetapan->id) }}" class="btn btn-outline-primary">
                            <i class="fas fa-edit me-1"></i> Edit Penetapan
                        </a>
                        <button class="btn btn-primary" onclick="toggleUploadForm()">
                            <i class="fas fa-paperclip me-1"></i> Upload Dokumen Cepat
                        </button>
                        <a href="{{ route('upload.spmi-penetapan', $penetapan->id) }}" class="btn btn-success">
